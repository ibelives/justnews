...
function filter_content($content=''){
    ...
    if(count($title)>0){
        ...
        for($x=0; $x<count($title); $x++){
            $html .='<li class="entry-tab-item j-tabs-item'.($x==0?' active':'').'">'.$title[$x].'</li>';
        }
        $html .= '</ul>';
        for($y=0; $y<count($sec); $y++){
            $html .='<div class="entry-tab-content j-tabs-wrap'.($y==0?' active':'').'">'.$sec[$y].'</div>';
        }
...
function filter_images_add_alt( $image = '', $alt='' ){
    ...
    if ( preg_match( $pattern, $image, $matches ) ) {
        if ( ! trim($matches[1]) ) {
            $image = preg_replace( '/(\<.+\s)alt=([\'"])(.+)/', sprintf( '$1alt=$2%s$3', $alt ), $image );
        }
    } else { // alt attribute not set.
        $image = preg_replace( '/(\<.+\s)src=([\'"])(.+)/', sprintf( '$1alt=$2%s$2 src=$2$3', $alt ), $image );
    }
    return $image;
}
